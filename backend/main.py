...
def get_study_buddies(id: str) -> bool:
    return jsonable_encoder(student_metrics.study_buddy_finder(id))
...
def suggest_next_semester_classes(id: str) -> bool:
    return jsonable_encoder(student_metrics.suggest_next_semester_classes(id, top_k=15))

# Return all nodes/data for a specific user
@app.get("/api/kg/{id}")
def get_id_data(id: str) -> dict:
    return jsonable_encoder(qu.fetch_student_data(id))

#Return all relations in a KG
@app.get("/api/kg/relations")
def get_all_relations() -> dict:
    return jsonable_encoder(qu.fetch_all_relations())

# Return all nodes/data in the KG
@app.get("/api/kg/")
def get_all_data() -> dict:
    return jsonable_encoder(qu.fetch_full_kg_data())
